...
test.describe("Skills Page Interactions", () => {
  ...
  test("hovering over a skill card transitions the icon from grayscale to color", async ({ page }) => {
    ...
    const initialFilter = await helmIcon.evaluate((el) => window.getComputedStyle(el).filter);
    ...
    await expect(async () => {
      const hoveredFilter = await helmIcon.evaluate((el) => window.getComputedStyle(el).filter);
      // It should either be 'none' or 'grayscale(0)' depending on the browser
      const isColor = hoveredFilter === 'none' || hoveredFilter.includes('grayscale(0)');
      expect(isColor).toBe(true);
      
      const hoveredOpacity = await helmIcon.evaluate((el) => window.getComputedStyle(el).opacity);
  ...
  test("icon returns to grayscale when hover is removed", async ({ page }) => {
    ...
    await helmCard.hover();
    await expect(async () => {
      const filter = await helmIcon.evaluate((el) => window.getComputedStyle(el).filter);
